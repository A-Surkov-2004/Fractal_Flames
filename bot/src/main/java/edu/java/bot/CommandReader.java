...
public class CommandReader {
    ...
    private static final int EXIT_CODE = 130;
    private static final String START_COMMAND = "/start";
    private static final String SHAPE_DESCRIPTION_BASE = "Draws specified shape or shaped lines";

    public CommandReader() {
        allExe.add(new StartCommandExecuter(START_COMMAND, "Start conversation with bot"));
        ...
        allMods.add(new AddWordExecuter("/diamond", SHAPE_DESCRIPTION_BASE));
        allMods.add(new AddWordExecuter("/disk", SHAPE_DESCRIPTION_BASE));
        allMods.add(new AddWordExecuter("/handkerchief", SHAPE_DESCRIPTION_BASE));
        allMods.add(new AddWordExecuter("/heart", SHAPE_DESCRIPTION_BASE));
        allMods.add(new AddWordExecuter("/horseshoe", SHAPE_DESCRIPTION_BASE));
        allMods.add(new AddWordExecuter("/hyperbolic", SHAPE_DESCRIPTION_BASE));
        allMods.add(new AddWordExecuter("/pdj", SHAPE_DESCRIPTION_BASE));
        allMods.add(new AddWordExecuter("/pillow", SHAPE_DESCRIPTION_BASE));
        allMods.add(new AddWordExecuter("/polar", SHAPE_DESCRIPTION_BASE));
        allMods.add(new AddWordExecuter("/sin", SHAPE_DESCRIPTION_BASE));
        allMods.add(new AddWordExecuter("/sphere", SHAPE_DESCRIPTION_BASE));
        allMods.add(new AddWordExecuter("/spiral", SHAPE_DESCRIPTION_BASE));
        allMods.add(new AddWordExecuter("/swirl", SHAPE_DESCRIPTION_BASE));
    ...
    public void read(Update update) {
        ...
        LOGGER.always().log("reading command: " + update.message().text());
        ...
        if (!userData.containsKey(id) || Objects.equals(userData.get(id).stateGet(), UserClass.DEFAULT_STATE)) {
            ...
            for (BasicCommandExecuter command : allExe) {
                if (Objects.equals(message, command.getName())) {
                    ...
                    if (!userData.containsKey(id) && !Objects.equals(message, START_COMMAND)) {
                        response =
                            (new SendMessage(
                                id,
                                "Вы не зарегестрированы в системе. Используйте команду " + START_COMMAND
                                    + ", чтобы начать."
                            ));
                    } else {
                        commandFound = true;
